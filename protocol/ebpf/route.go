...
func (i *Inbound) selectRedirectPrefixes() error {
	...
	i.redirectIPv4Prefix, err = selectRedirectPrefix(loopback.Attrs().Index, unix.AF_INET, redirectIPv4Candidates)
	if err != nil {
		return E.Cause(err, "select internal IPv4 redirect prefix")
	}
	if i.requiresIPv6Redirect() {
		i.redirectIPv6Prefix, err = selectRedirectPrefix(loopback.Attrs().Index, unix.AF_INET6, redirectIPv6Candidates)
		if err != nil {
...
func selectRedirectPrefix(loopbackIndex int, family int, candidates []netip.Prefix) (netip.Prefix, error) {
