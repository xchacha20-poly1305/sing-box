...
func (i *Inbound) logMissingTCPRedirect(ctx context.Context, backend *ECommon.CgroupBackend, listener netip.AddrPort) {
	...
	message := "missing TCP redirect state for valid token"
	if !i.isRedirectListenerDestination(listener) {
		message = "unexpected direct connection to eBPF listener"
	}
	args := []any{
		message,
		": listener=", listener,
...
func (i *Inbound) isRedirectListenerDestination(destination netip.AddrPort) bool {
	if !destination.IsValid() || destination.Port() != i.listeners.selectedPort() {
		return false
	}
