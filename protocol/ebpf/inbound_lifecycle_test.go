...
func TestTakeCgroupBackendRetiresCleanupAccess(t *testing.T) {
	...
	if cleanupErrors := inbound.diagnostics.localUDPCleanupError.Load(); cleanupErrors != 0 {
		t.Fatalf("retired backend cleanup was reported as an error: %d", cleanupErrors)
	}
}
