...
func (s *internalListenerSet) registerTCPAssignmentSockets(backend *ECommon.SharedNetworkBackend) error {
	...
	if s.tcp4 != nil {
		registrations[0].listener = s.tcp4.TCPListener()
	}
	if s.tcp6 != nil {
		registrations[1].listener = s.tcp6.TCPListener()
	}
