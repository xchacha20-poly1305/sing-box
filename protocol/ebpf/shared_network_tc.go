...
func (m *sharedTCManager) reconcile() error {
	...
	for interfaceName, attachment := range m.attachments {
		...
		if loaded && link.Attrs().Index == attachment.interfaceIndex {
			repaired, repairErr := repairSharedTC(link, attachment, m.backend, m.priority)
			if repairErr != nil {
				return E.Cause(repairErr, "verify shared-network TC attachment on ", interfaceName)
			}
			if repaired {
				m.logger.Warn("eBPF shared-network TC attachment was removed or replaced; reattached to ", interfaceName)
			}
			continue
		}
