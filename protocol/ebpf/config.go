...
func normalizeDNSMode(mode string) (string, error) {
	...
	case dnsModeRespectPolicy, dnsModeOff:
		return mode, nil
	default:
...
func validateSharedOptions(enabled bool, options option.EBPFSharedOptions) error {
	...
	if options.DNSMode != "" || len(options.Interface) > 0 || options.IPv6Mode != "" || options.BypassPrivateAddress != nil ||
		...
		options.Advanced.DataPlane != "" || options.Advanced.RoutingMark != 0 || options.Advanced.RoutingTable != 0 {
...
func normalizeSharedNetworkOptions(options option.EBPFSharedOptions) (option.EBPFSharedOptions, error) {
	...
	switch options.Advanced.DataPlane {
	case "", sharedNetworkDataPlaneAuto:
		options.Advanced.DataPlane = sharedNetworkDataPlaneAuto
	case sharedNetworkDataPlaneSocketAssign, sharedNetworkDataPlaneRewrite:
	default:
		return option.EBPFSharedOptions{}, E.New("unknown shared.advanced.data_plane: ", options.Advanced.DataPlane)
	}
	if options.Advanced.DataPlane == sharedNetworkDataPlaneRewrite &&
		(options.Advanced.RoutingMark != 0 || options.Advanced.RoutingTable != 0) {
		return option.EBPFSharedOptions{}, E.New(
			"shared.advanced.routing_mark and routing_table require data_plane auto or socket_assign",
		)
	}
	if options.Advanced.RoutingTable > 1<<31-1 {
		return option.EBPFSharedOptions{}, E.New("shared.advanced.routing_table exceeds the Linux routing table range")
	}
