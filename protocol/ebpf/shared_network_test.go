...
func TestNormalizeSharedNetworkDataPlane(t *testing.T) {
	base := option.EBPFSharedOptions{Interface: []string{"wlan0"}}
	normalized, err := normalizeSharedNetworkOptions(base)
	if err != nil {
		t.Fatal(err)
	}
	if normalized.Advanced.DataPlane != sharedNetworkDataPlaneAuto {
		t.Fatalf("unexpected default shared data plane: %s", normalized.Advanced.DataPlane)
	}
	rewrite := base
	rewrite.Advanced.DataPlane = sharedNetworkDataPlaneRewrite
	rewrite.Advanced.RoutingMark = 1
	if _, err = normalizeSharedNetworkOptions(rewrite); err == nil {
		t.Fatal("rewrite data plane accepted socket-assignment routing options")
	}
	invalid := base
	invalid.Advanced.DataPlane = "invalid"
	if _, err = normalizeSharedNetworkOptions(invalid); err == nil {
		t.Fatal("invalid shared data plane was accepted")
	}
}
