...
type Endpoint struct {
	...
	deviceOptions      *easyconnecttransport.DeviceOptions
...
func NewEndpoint(ctx context.Context, router adapter.Router, logger log.ContextLogger, tag string, options option.EasyConnectEndpointOptions) (adapter.Endpoint, error) {
	...
	easyConnectEndpoint.deviceOptions = &easyconnecttransport.DeviceOptions{
		Context:         ctx,
		Logger:          logger,
...
func (e *Endpoint) Start(stage adapter.StartStage) error {
	if stage == adapter.StartStateInitialize {
		...
		device, err := easyconnecttransport.NewDevice(*e.deviceOptions)
		if err != nil {
			return err
		}
