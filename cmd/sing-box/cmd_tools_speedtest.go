...
func init() {
	...
	commandSpeedTest.Flags().BoolVar(&commandSpeedTestQuite, "quite", false, "quite mode")
	commandSpeedTest.Flags().Uint32Var(&commandSpeedTestDataSize, "data-size", math.MaxUint32, "Data size for download and upload tests.")
...
func downloadTest(ctx context.Context, dialer N.Dialer) error {
	...
	if err != nil {
		if E.IsCanceled(err) {
			if downloaded > 0 {
				log.Info("download incomplete:\n downloaded:", downloaded, " speed: ", formatSpeed(downloaded, time.Since(started)))
			}
			return nil
		}
		return err
...
func uploadTest(ctx context.Context, dialer N.Dialer) error {
	...
	if err != nil {
		if E.IsCanceled(err) {
			if uploaded > 0 {
				log.Info("upload incomplete:\n uploaded:", uploaded, " speed: ", formatSpeed(uploaded, time.Since(started)))
			}
			return nil
		}
		return err
