...
func NewGRPCMultiConn(service GunMultiService, cancel context.CancelCauseFunc) net.Conn {
	conn := &GRPCMultiConn{
		GunMultiService: service,
		cancel:          cancel,
	}
	if client, isClient := service.(grpc.ClientStream); isClient {
		return &GRPCMultiClientConn{GRPCMultiConn: conn, client: client}
	}
	return conn
}
...
var _ N.WriteCloser = (*GRPCMultiClientConn)(nil)

type GRPCMultiClientConn struct {
	*GRPCMultiConn
	client grpc.ClientStream
}

func (c *GRPCMultiClientConn) CloseWrite() error {
	return c.client.CloseSend()
}
