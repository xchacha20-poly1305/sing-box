...
See [LOCAL.md](LOCAL.md) for standard Linux and rooted Android procedures.
...
- log startup and shutdown runtime status, map occupancy, program IDs, attachment
  state, UDP misses, recovery counters, and maintenance timing;
- enable kernel `BPF_STATS_RUN_TIME` when supported, exposing run count, total
  runtime, and average ns/run for each BPF program.
